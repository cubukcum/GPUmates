...
$LauncherBuild = Join-Path $ProjectRoot 'installer\coordinator\Build-CoordinatorLauncher.ps1'
$InstallerSource = Join-Path $PSScriptRoot 'GPUmatesUnified.iss'
...
& $LauncherBuild
if ($LASTEXITCODE -ne 0) { throw "Coordinator launcher build failed with exit code $LASTEXITCODE." }
...
if ($NewerSources.Count -gt 0) {
    throw "The Control Center web build is stale. Run pnpm run build:control first. Newer or missing source: $($NewerSources -join ', ')"
}
$IndexText = Get-Content -LiteralPath $StaticIndex -Raw
if ($IndexText -match '__GPUMATES_CONTROL_TOKEN__|gpumates-control-token') {
    throw 'The Control Center index contains the legacy unauthenticated token placeholder.'
}
...
$OutputPath = Join-Path $ProjectRoot ("dist\installer\GPUmates-Setup-{0}.exe" -f $AppVersion)
